...
\begin{frame}
\begin{center}
\only<1>{\large{Common concept:\\ Complicated model $\rightarrow$ Realistic simulations.\\}}\only<2-3>{\large{Systematic approach:\\ Simplified models $\rightarrow$ Linking to data $\rightarrow$ Meaningful perditions.\\}}
\end{center}
...
%  	\item Ensure identifiability of models. 
%  	\end{itemize}}
\end{frame}
%% Chapters 3-5
...
\begin{column}{0.5\textwidth}
	...
	\vfil
	\vspace{0.3cm}
	\only<1>{\includegraphics[scale=0.2]{Figures/grid2_heatmap.png}}
	\only<2>{\includegraphics[scale=0.2]{Figures/grid_mask.png}}\\
...
\end{equation*}
\vspace{0.3cm}
SMM is linear in $\Theta$ and non-linear in $\mathbfit{x}$.   
\end{frame}
\begin{frame}{Approximate ML estimation}
E-step:
\vspace{-0.5cm}
\begin{equation*}
...
\begin{figure}
	...
	\draw[line width=0.5mm,cyan!70!black] (3.55,7.3) rectangle (7.95,1.8);
	\node at (3.7,6.2) (A9){};
	\node at (3.7,2.8) (A10){};
	\node at (3.7,4.45) (A11){};
	\node at (7.8,5.8) (A12){};
...
fluorescence intensity obtained from the imaging data is proportional to the local PIP$_3$ concentration.
	\item The cell is a 2-D curve $\Gamma_{\td}$.
...
\begin{columns}
	...
	\begin{column}{0.5\textwidth}
		\centering
		\begin{itemize}
			\item Very weak correlation between between $a_{\td-1}^{k}$ and $\mathbfit{v}_{\td}^{k}$ for all cells;
			\item PIP$_3$ does not activate protrusion growth;
			\item Mann-Whitney test results: on average, higher concentrations of PIP$_3$ accelerate protrusion growth.
		\end{itemize}
	\end{column}
\end{columns}
\end{frame}
